...
run(LV2_Handle instance, uint32_t n_samples)
{
    ...
    // Feed audio to pitch tracker (need non-const pointer)
    float* input_copy = (float*)malloc(n_samples * sizeof(float));
    if (input_copy) {
        memcpy(input_copy, self->input, n_samples * sizeof(float));
        self->tracker->add(n_samples, input_copy);
        free(input_copy);
    }
